...
begin
  ...
  -- Reopen locked weeks before the legacy importer replaces their CTS rows.
  -- Include rows from an earlier copy of the same file in case its matching was changed.
  for v_timesheet in
    select r.*
  ...
  -- Reuse the canonical review workflow for totals, reconciliation, snapshots,
  -- audit events, approval, and locking.
  for v_affected in
    select distinct
      (value->>'cts_job_candidate_id')::uuid as candidate_id,
      (value->>'week_ending_date')::date - 6 as week_start_date

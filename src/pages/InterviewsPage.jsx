    ...
    <div style={{ padding: 24, fontFamily: "Inter, sans-serif", background: "#f8fafc", minHeight: "100vh" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gap: 20 }}>
        <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 20, padding: 24 }}>
          <h1 style={{ margin: 0, fontSize: 32, fontWeight: 900 }}>Interview History</h1>
          ...
          {loading ? (
            ...
            filteredRows.map((row) => (
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
                  <div>
                    ...
                    <div style={{ marginTop: 6, color: "#64748b", fontSize: 14 }}>
                      {row.interview_date || "No date"} • {row.email || "No email"} • {row.phone || "No phone"}
                    </div>
                  </div>

                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: 28, fontWeight: 900 }}>{row.total_score}</div>
                    <div style={{ color: "#334155", fontWeight: 700 }}>{row.classification}</div>
                    <button
                ...
                {expandedId === row.id && (
                  <div style={{ marginTop: 18, paddingTop: 18, borderTop: "1px solid #e2e8f0", display: "grid", gap: 14 }}>
                    <div>
                    ...
                    {Array.isArray(row.answers_snapshot) && row.answers_snapshot.length > 0 && (
                      <div style={{ display: "grid", gap: 12 }}>
                          ...
                          >
                            ...
                            <div style={{ display: "grid", gap: 10 }}>
                              {section.questions?.map((q, qIdx) => (
                                <div key={`${row.id}-${idx}-${qIdx}`} style={{ borderTop: qIdx ? "1px solid #e2e8f0" : "none", paddingTop: qIdx ? 10 : 0 }}>
                                  <div style={{ fontWeight: 700 }}>{q.questionPrompt}</div>
                                  <div style={{ marginTop: 4, color: "#334155" }}>
                                    <strong>Answer:</strong> {q.selectedOption}
                                  </div>
                                  {q.isOther && q.otherText ? (
                                    <div style={{ marginTop: 4, color: "#334155", whiteSpace: "pre-wrap" }}>

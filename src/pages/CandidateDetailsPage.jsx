...
import { BriefcaseBusiness, CalendarDays, CheckCircle2, Copy, FileText, Link2, Loader2, Mail, MapPin, MessageCircle, Phone } from "lucide-react";
...
export default function CandidateDetailsPage() {
  ...
  const completeVetting = async () => {
    if (!canEdit || saving || worker?.vetting_completed_at) return;
    setSaving("vetting"); setFeedback("");
    const { data, error } = await supabase.rpc("complete_candidate_vetting", { p_worker_id: workerId });
    if (error) setFeedback(error.message || "Could not complete candidate vetting.");
    else {
      setWorker((current) => ({ ...current, vetting_completed_at: data }));
      setFeedback("Vetting completed. Andrea, Carlos, and Enmanuel were notified.");
    }
    setSaving("");
  };
